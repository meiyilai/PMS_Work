...
public class PmsFile {
	...
	public final int offsetRes1 = 40; // 保留 4字节
	public final int offsetRes2 = 44; // 保留 4字节
	public final int offsetRes3 = 48; // 保留 4字节
	public final int offsetRes4 = 52; // 保留 4字节
	public final int offsetRes5 = 56; // 保留 4字节
	// 录波数据 录波数据 N字节
	// 图片数据 图片数据 M字节
	// 菜谱描述 菜谱文字描述 K字节
	...
	public void setTipsTimeNode(int index, String tips,int[] foodIDs, String[] foodNames,
			int[] foodWgts, int time,byte[] bufTime) {
	...
	private static int ConvertToint(byte[] bufT, int offset) {
		...
		value = (int) ((bufT[offset] & 0xFF) | ((bufT[offset + 1] & 0xFF) << 8)
				| ((bufT[offset + 2] & 0xFF) << 16) | ((bufT[offset + 3] & 0xFF) << 24));
		return value;
	}
	...
	int readReg(int offset) {
		...
		value = (int) ((PMS_Data[offset] & 0xFF)
				...
				| ((PMS_Data[offset + 2] & 0xFF) << 16) | ((PMS_Data[offset + 3] & 0xFF) << 24));
		return value;
	}
